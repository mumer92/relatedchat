...
import { useDetailByChat, useTheme, useUserById } from 'lib/hooks';
...
function DirectMessage({ dm }: { dm: any }) {
  ...
  const { value } = useUserById(otherUserId || null, true);
  ...
  const notifications =
    dm && detail ? dm.lastMessageCounter - detail.lastRead : 0;
      ...
      >
        <div className="relative mr-2 flex-shrink-0">
          ...
          >
            <div
              style={{
                backgroundColor: (isMe ? true : value?.isPresent)
                  ? '#94e864'
                  : 'transparent',
              }}
              className={classNames(
        ...
        >
          {!isMe ? value?.displayName : `${userdata?.displayName} (me)`}
        </div>
      </div>
      <div className="mr-3">
